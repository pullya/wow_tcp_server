...
func Test_validateClientsCount(t *testing.T) {
	t.Parallel()
	type args struct {
		in string
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := validateClientsCount(tt.args.in)
			if (err != nil) != tt.wantErr {
				t.Errorf("validateClientsCount() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("validateClientsCount() = %v, want %v", got, tt.want)
			}
		})
	}
}
